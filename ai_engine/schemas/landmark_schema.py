...
class HandTelemetryData(BaseModel):
    ...
    landmarks: List[Point3D] = Field(default_factory=list, description="21 landmarks list")
    confidence: float = Field(default=0.0, description="MediaPipe confidence")
    center: Point3D = Field(default_factory=lambda: Point3D(x=0, y=0, z=0), description="Geometric center point")
    bounding_box: Optional[BoundingBox3D] = Field(default=None, description="3D Bounding bounds")

class PoseTelemetryData(BaseModel):
    present: bool = Field(..., description="Whether pose is in frame")
    landmarks: List[Point3D] = Field(default_factory=list, description="33 keypoints list")
    confidence: float = Field(default=0.0, description="MediaPipe confidence")
    ...
    torso_rotation: float = Field(default=0.0, description="Body alignment projection angle")

class FaceTelemetryData(BaseModel):
    present: bool = Field(..., description="Whether face is in frame")
    landmarks: List[Point3D] = Field(default_factory=list, description="Mesh landmarks list")
    confidence: float = Field(default=0.0, description="MediaPipe confidence")
