...
class SampleValidator:
    def __init__(self, min_sequence_len: int = 10, max_sequence_len: int = 120, min_visibility: float = 0.3):
        self.min_sequence_len = min_sequence_len
        self.max_sequence_len = max_sequence_len
        self.min_visibility = min_visibility

    def validate_frame(self, flat_landmarks: Union[np.ndarray, List[float]]) -> Tuple[bool, str]:
        """
    ...
    def validate_sequence(self, sequence: List[np.ndarray], min_vis_ratio: float = 0.5) -> Tuple[bool, str]:
        ...
        if seq_len < self.min_sequence_len:
            return False, f"Sequence too short: got {seq_len} frames, min is {self.min_sequence_len}"
        if seq_len > self.max_sequence_len:
            return False, f"Sequence too long: got {seq_len} frames, max is {self.max_sequence_len}"
        ...
        if (visible_frames / seq_len) < min_vis_ratio:
            return False, f"Visibility tracking is too poor: only {visible_frames}/{seq_len} frames are visible"
