...
class Trainer:
    ...
    def train_model(
        ...
        train_loader = DataLoader(
            GestureDataset(X_train, y_train), batch_size=batch_size, shuffle=True, pin_memory=True
        )
        val_loader = DataLoader(GestureDataset(X_val, y_val), batch_size=batch_size, shuffle=False, pin_memory=True)
        test_loader = DataLoader(GestureDataset(X_test, y_test), batch_size=batch_size, shuffle=False, pin_memory=True)
        ...
        metrics = evaluator.evaluate_predictions(
            np.array(test_targets), np.array(test_preds), np.array(test_probs), classes
        )
        ...
        torch.save(best_model_state, model_path)  # nosemgrep: trailofbits.python.pickles-in-pytorch.pickles-in-pytorch
