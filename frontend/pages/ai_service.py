...
class AIService:
    def __init__(self):
        ...
        if settings.GOOGLE_TRANSLATE_API_KEY:
            try:
                ...
                self.google_translate_client = translate.Client(api_key=settings.GOOGLE_TRANSLATE_API_KEY)
        ...
        # if settings.DEFAULT_INFERENCE_MODE == 'local' and settings.OLLAMA_API_BASE_URL:
        #     try:
        #         import ollama
        #         self.ollama_client = ollama.Client(host=settings.OLLAMA_API_BASE_URL)
        #         logger.info(f"Ollama client initialized for model: {settings.OLLAMA_MODEL_NAME}")
        #     except ImportError:
    ...
    def query_ollama(self, prompt: str, model: str = settings.OLLAMA_MODEL_NAME) -> str:
